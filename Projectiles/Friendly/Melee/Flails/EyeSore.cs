...
namespace OrsonsMod.Projectiles.Friendly.Melee.Flails
{
	public class EyeSore : ModProjectile
	{
		// The folder path to the flail chain sprite
		private const string ChainTexturePath = "OrsonsMod/Projectiles/Friendly/Melee/Flails/EyeSoreChain";
		private int dashCD;
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Eye Sore"); // Set the projectile name to Example Flail Ball
		}
		...
		private int NearbyEnemies() 
		{
			...
			int target = -1;
			for (int i = 0; i < 200; i++)
			{ if (Main.npc[i].active && Main.npc[i].CanBeChasedBy(this,false))
				{
					if (Vector2.Distance(projectile.Center, Main.npc[i].Center) < targetmag)
					{ 
						targetmag = Vector2.Distance(projectile.Center, Main.npc[i].Center);
						target = i;
					}
				}
			}
		...
		public override void AI()
		{
			...
			else if (projectile.ai[0] == 1f)
			{
				...
				if (!player.channel || Vector2.Distance(projectile.Center, player.Center) > 300)
				{
					...
					else if(Vector2.Distance(projectile.Center, player.Center) > 600)
					{ projectile.velocity = (player.Center - projectile.Center) * (20 / Vector2.Distance(player.Center, projectile.Center)); }
			...
			if (dashCD > 0) { dashCD -= 1; projectile.velocity *= 0.98f; }
			// Spawn some dust visuals
			int target = NearbyEnemies();
			if (target != -1 && dashCD <= 0)
			{
				{ projectile.velocity = (Main.npc[target].Center - projectile.Center) * (20 / Vector2.Distance(projectile.Center, Main.npc[target].Center)); dashCD = 30; }
			}
		}
